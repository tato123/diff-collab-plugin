...
const FormHelper = require("get-set-form-values");
...
<form method="dialog">
    <h1 class="h1">
        ...
        ${
          icon
            ? `<img ${
                usingPluginIcon
                  ? `class="plugin-icon" title="${manifest.name}"`
                  : ""
              } src="${icon}" />`
            : ""
        }
  ...
  try {
    ...
    if (response === "reasonCanceled") {
      ...
      if (formValues) {
        const a = new FormHelper();
        const retFormValues = _.pickBy(a.get(form), _.identity);
